"""Dense IG carousel — Timing giorni del mese (esclusiva Leona.Lab)."""
...
GALLERIA = ROOT / "assets" / "instagram" / "galleria-giorni-mese"
...
def slide_01():
    ...
    y = 40
    d.text(
        ((W - text_w(d, "SOLO SU LEONA.LAB", font(28, True))) // 2, y),
        "SOLO SU LEONA.LAB",
        font=font(28, True),
        fill=GOLD,
    )
    y += 70
    for line in wrap(d, "Il giorno del mese che conta davvero", font(64, True), 860):
    ...
    y = body_lines(
        ...
        y,
        [
            "Sai che agosto è forte.",
            "Ma quale giorno del mese",
            "ha premiato di più long e short?",
            "Il timing fine, su dati reali.",
        ],
        size=36,
        fill=MUTED,
        gap=50,
    ...
    finalize(img, OUT / "ig-giorni-01-cover.png", "1 / 7")


def slide_02():
    img = Image.new("RGB", (W, H), BG)
    d = ImageDraw.Draw(img)
    y = title_block(d, 20, "01  ·  COSA È", "Timing dentro il mese")
    y = rule(d, y)
    y = body_lines(
        d,
        y,
        [
            "Per ogni mese, su storico reale dell’asset,",
            "indichiamo il giorno più favorevole al long",
            "e quello più favorevole allo short.",
        ],
        size=36,
        gap=50,
    )
    y += 16
    items = [
        ("Giorno Long", "Quando il mese ha storicamente spinto"),
        ("Giorno Short", "Quando il mese ha storicamente pesato"),
        ("Bias del mese", "Il vento di fondo, mese per mese"),
    ]
    for title, sub in items:
        ...
        y += 128
    finalize(img, OUT / "ig-giorni-02-cosa-e.png", "2 / 7")


def slide_03():
    img = Image.new("RGB", (W, H), BG)
    d = ImageDraw.Draw(img)
    y = title_block(d, 20, "02  ·  IL PROBLEMA", "Il mese non basta")
    y = rule(d, y)
    y = body_lines(
        d,
        y,
        [
            "Due trade nello stesso mese non valgono",
            "lo stesso se uno cade sul giorno forte",
            "e l’altro sul giorno debole.",
        ],
        size=36,
        gap=52,
    )
    ...
    d.text((120, y + 36), "Cosa rischi senza il giorno", font=font(34, True), fill=GOLD)
    for i, line in enumerate(
        [
            "• entrare nel giorno sbagliato del mese",
            "• forzare setup contro il timing fine",
            "• leggere solo la media mensile",
            "• perdere il quando operativo",
        ]
    ):
        d.text((120, y + 100 + i * 48), line, font=font(32), fill=WHITE)
    finalize(img, OUT / "ig-giorni-03-perche.png", "3 / 7")


def slide_04():
    img = Image.new("RGB", (W, H), BG)
    d = ImageDraw.Draw(img)
    y = title_block(d, 10, "03  ·  COME SI LEGGE", "Long day vs Short day", wrap_w=860)
    y = rule(d, y)
    card(
        d,
        70,
        ...
        230,
        "Giorno Long",
        "Il giorno del mese con media e hit-rate storicamente più favorevoli al rialzo. Cerca long con più pazienza.",
        GREEN,
        38,
        30,
    )
    ...
    card(
        ...
        230,
        "Giorno Short",
        "Il giorno del mese storicamente più favorevole al ribasso. Non è un ordine: è un avviso di timing.",
        RED,
        38,
        30,
    )
    ...
    body_lines(
        d,
        y,
        ["Allinea giorno e bias del mese."],
        size=32,
        fill=GOLD,
        gap=44,
        bold=True,
    )
    finalize(img, OUT / "ig-giorni-04-lettura.png", "4 / 7")


def slide_05():
    img = Image.new("RGB", (W, H), BG)
    d = ImageDraw.Draw(img)
    y = title_block(d, 10, "04  ·  DATI REALI", "Niente numeri inventati")
    y = rule(d, y)
    y = body_lines(
        d,
        y,
        [
            "Rendimenti giornalieri reali,",
            "mese per mese, Gennaio–Dicembre.",
            "Aggiornati al cambio asset.",
        ],
        size=36,
        gap=50,
    )
    ...
    d.text((120, y + 30), "Nella pagina Stagionalità trovi", font=font(34, True), fill=GOLD)
    for i, line in enumerate(
        [
            "• bias del mese (long / short)",
            "• giorno long più probabile",
            "• giorno short più probabile",
            "• media e hit-rate su storico reale",
            "• * = campione debole, da filtrare",
        ]
    ):
        d.text((120, y + 95 + i * 44), line, font=font(30), fill=WHITE)
    finalize(img, OUT / "ig-giorni-05-dati.png", "5 / 7")


def slide_06():
    img = Image.new("RGB", (W, H), BG)
    d = ImageDraw.Draw(img)
    y = title_block(d, 10, "05  ·  COME USARLO", "Giorno = quando, non quanto")
    y = rule(d, y)
    steps = [
        ("1", "Leggi il bias del mese", "Vento di fondo long o short"),
        ("2", "Apri i giorni", "Timing long / short del mese"),
        ("3", "Incrocia il desk", "COT, valuation, poi il segnale"),
    ]
    for num, title, sub in steps:
        ...
        y += 168
    finalize(img, OUT / "ig-giorni-06-come-usarlo.png", "6 / 7")
...
def slide_07():
    ...
    y += 70
    for line in wrap(d, "Vuoi il giorno giusto sul desk?", font(60, True), 860):
    ...
    y = body_lines(
        ...
        y,
        [
            "Solo su Leona.Lab: bias del mese,",
            "giorno long, giorno short — dati reali.",
            "Il quando, senza lasciare il desk.",
        ],
        size=36,
        gap=52,
    )
    ...
    y += 160
    body_lines(d, y, ["Timing giorni · esclusiva · contesto"], size=28, fill=MUTED, gap=44)
    finalize(img, OUT / "ig-giorni-07-cta.png", "7 / 7")


def copy_to_galleria():
    GALLERIA.mkdir(parents=True, exist_ok=True)
    mapping = [
        ("ig-giorni-01-cover.png", "ig-galleria-01.png"),
        ("ig-giorni-02-cosa-e.png", "ig-galleria-02.png"),
        ("ig-giorni-03-perche.png", "ig-galleria-03.png"),
        ("ig-giorni-04-lettura.png", "ig-galleria-04.png"),
        ("ig-giorni-05-dati.png", "ig-galleria-05.png"),
        ("ig-giorni-06-come-usarlo.png", "ig-galleria-06.png"),
        ("ig-giorni-07-cta.png", "ig-galleria-07.png"),
    ]
    for src, dst in mapping:
        data = (OUT / src).read_bytes()
        (GALLERIA / dst).write_bytes(data)
        print(f"galleria {dst}")
